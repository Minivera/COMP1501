...
class Enemy(pygame.sprite.Sprite):
    ...
    def set_image(self):
        ...
        tank = pygame.image.load(os.path.join(assets_folder, 'tank_red.png')).convert_alpha()
        tank = pygame.transform.scale(tank, (self.square_size[0] // 2, self.square_size[1] // 2))

        # rotate the tank according ot its direct
        tank_angle = math.atan2(self.velocity[1], self.velocity[0])
        tank = pygame.transform.rotate(tank, math.degrees(tank_angle) - 90)

        self.image.blit(tank, (0, 0))
